...
# Prepare data for the copy task
train_dataset = SequenceDataset(vocab_size, seq_length, 1000, task='copy')
val_dataset = SequenceDataset(vocab_size, seq_length, 200, task='copy')
...
trainer.train(train_loader, val_loader, num_epochs, save_path='model_copy.pth')

# Test on a few examples
test_data = generate_test_sequences(vocab_size, seq_length, 5, task='copy')
model.eval()
...
while True:
    ...
    try:
        ...
        with torch.no_grad():
            output, _ = model(input_seq)
            pred = torch.argmax(output, dim=-1)[0]
        print("Predicted copy:", [str(i.item()) for i in pred])
